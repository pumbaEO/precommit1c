...
Функция Компилировать(Знач Путь, Знач КаталогВыгрузки, Знач ТипФайла)
    ...
    Пока СтрокаПереименования <> Неопределено Цикл
    	...
    	СписокСтрок = СтроковыеФункции.РазложитьСтрокуВМассивПодстрок(СтрокаПереименованияВрем, "-->");
    	...
    	СтарыйПуть = ОбъединитьПути(ПутьКИсходникам, СтрЗаменить(СписокСтрок[0], "\\", ПолучитьРазделительПути()));
    ...
    ПутьЗапаковщика = ПолучитьПутьЗапаковщика("v8unpack");
    Если НЕ ЗначениеЗаполнено(ПутьЗапаковщика) Тогда
    	ПутьЗапаковщика = ПолучитьПутьЗапаковщика("unpackv8");
    КонецЕсли;

    Ожидаем.Что(ПутьЗапаковщика, "Не найден путь к v8unpack").Не_().ИмеетДлину(0);

    КомандаЗапуска = "%1 -B ""%2"" ""%3""";
    КомандаЗапуска = СтроковыеФункции.ПодставитьПараметрыВСтроку(КомандаЗапуска, ПутьЗапаковщика, ВременныйКаталог, ВременныйФайл);
	...
	Ожидаем.Что(Процесс.КодВозврата, "Не удалось упаковать каталог " + ВременныйКаталог + Символы.ПС + Процесс.ПотокВывода.Прочитать()).Не_().Равно(0);

	Лог.Отладка(СтроковыеФункции.ПодставитьПараметрыВСтроку("Копирование из %1 в %2", ВременныйФайл, КаталогВыгрузки));
	ГотовыйФайл = ОбъединитьПути(КаталогВыгрузки, ВременныйФайл.Имя);
	ПереместитьФайл(ВременныйФайл, ГотовыйФайл);
...
Функция ПолучитьПутьЗапаковщика(ИмяФайла)
	Возврат "";
КонецФункции
